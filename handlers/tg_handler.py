...
bot_token = os.getenv('BOT_TG_API')
...
guitar_agent = Agent(
    ...
    session_id='acoustic_guitar_helper_tg',
    db=db,
...
async def start_message(message: Message):
    await message.answer(
            '''
            Привет, я твой помощник в выборе Акустических гитар!
            '''
        )

@dp.message()
async def any_message(user_message):
    response = guitar_agent.run(user_message).content
    await user_message.answer(response)
